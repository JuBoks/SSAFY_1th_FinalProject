...
async function apiRegistFavoriteArea(payload, success, fail) {
  await api.post(`/favoritearea/${payload.userId}`, JSON.stringify(payload)).then(success).catch(fail);
}
...
async function apiDeleteFavoriteArea(payload, success, fail) {
  await api.delete(`/favoritearea/${payload.userId}/${payload.dongCode}`).then(success).catch(fail);
}
